...
export class PutHolidayService {
  ...
  async handleMovableHoliday(
    holidayType: HolidayType,
    holidaysInResolvedDate: Holiday[],
    state: string,
    city: string,
    resolvedDate: string,
  ): Promise<Holiday> {
    if (holidayType === HolidayType.ESTADUAL) {
      throw new HttpException(
        'Não é possível adicionar um feriado móvel no estado',
        HttpStatus.BAD_REQUEST,
      );
    }

    const exists = holidaysInResolvedDate.some((h) => h.city === city);

    if (
      exists ||
      resolvedDate === MovableHolidays.SEXTA_FEIRA_SANTA ||
      resolvedDate === MovableHolidays.PASCOA
    ) {
  ...
  async hanldeStateHoliday(
    holidayExistsInState: Holiday | undefined,
    newHoliday: CreateHolidayDto,
    holidaysInResolvedDate: Holiday[],
  ): Promise<Holiday | null | undefined> {
    ...
    return holidayExistsInState.id
      ? await this.holidaysService.update(holidayExistsInState.id, newHoliday)
      : undefined;
  }

  async handleMunicipalHoliday(
    holidayExistsInCity: Holiday | undefined,
    holidayExistsInState: Holiday | undefined,
    newHoliday: CreateHolidayDto,
  ): Promise<Holiday | null | undefined> {
